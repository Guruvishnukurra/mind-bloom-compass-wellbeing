      ...
      <CardHeader className="pb-3 bg-gradient-to-r from-lavender-500 to-deep-ocean-400 text-white">
        <div className="flex justify-between items-center">
          ...
          <Button 
            ...
            className="h-9 text-white border-white/30 hover:bg-white/20 hover:text-white"
          >
        ...
        <ScrollArea className="flex-1 px-6 py-5" ref={scrollAreaRef}>
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8">
              <div className="w-20 h-20 rounded-full bg-lavender-100 flex items-center justify-center mb-5 shadow-md">
                <Bot className="h-10 w-10 text-lavender-600" />
              </div>
              <h3 className="text-2xl font-heading font-medium mb-3 text-deep-ocean-500">Welcome to Your Mindful Assistant</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-md">
                {suggestedPrompts.map((prompt, index) => (
                  <Button
                    key={index}
                    variant="outline"
                    className="justify-start h-auto py-3 px-4 text-left border-lavender-200 hover:bg-lavender-50 hover:border-lavender-300 transition-all"
                    onClick={() => {
                      setInput(prompt);
                      inputRef.current?.focus();
                    }}
                  >
                    <MessageSquare className="h-4 w-4 mr-2 flex-shrink-0 text-lavender-500" />
          ...
          {isLoading && (
            <div className="flex justify-start gap-3 mb-5">
              ...
              <div className="rounded-2xl p-4 max-w-[80%] shadow-sm bg-white border border-lavender-200">
                <div className="flex gap-2">
                  <div className="w-2 h-2 rounded-full bg-lavender-500 animate-bounce" style={{ animationDelay: '0ms' }}></div>
                  <div className="w-2 h-2 rounded-full bg-lavender-500 animate-bounce" style={{ animationDelay: '150ms' }}></div>
                  <div className="w-2 h-2 rounded-full bg-lavender-500 animate-bounce" style={{ animationDelay: '300ms' }}></div>
                </div>
              </div>
            </div>
          )}
