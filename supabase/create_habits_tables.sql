...
CREATE POLICY IF NOT EXISTS select_own_habits ON public.habits
  FOR SELECT USING (auth.uid() = user_id);

-- Create policy to allow users to insert their own habits
CREATE POLICY IF NOT EXISTS insert_own_habits ON public.habits
  FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Create policy to allow users to update only their own habits
CREATE POLICY IF NOT EXISTS update_own_habits ON public.habits
  FOR UPDATE USING (auth.uid() = user_id);

-- Create policy to allow users to delete only their own habits
CREATE POLICY IF NOT EXISTS delete_own_habits ON public.habits
...
CREATE POLICY IF NOT EXISTS select_own_habit_completions ON public.habit_completions
  FOR SELECT USING (auth.uid() = user_id);

-- Create policy to allow users to insert their own habit completions
CREATE POLICY IF NOT EXISTS insert_own_habit_completions ON public.habit_completions
  FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Create policy to allow users to update only their own habit completions
CREATE POLICY IF NOT EXISTS update_own_habit_completions ON public.habit_completions
  FOR UPDATE USING (auth.uid() = user_id);

-- Create policy to allow users to delete only their own habit completions
CREATE POLICY IF NOT EXISTS delete_own_habit_completions ON public.habit_completions
  FOR DELETE USING (auth.uid() = user_id);
